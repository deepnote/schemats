...
describe('PostgresDatabase', () => {
    ...
    describe('getTableDefinition', () => {
        ...
        it('handles response correctly', async () => {
            ...
            const callback = db.each.getCall(0).args[2]
            const dbResponse = [
                {column_name: 'col1', udt_name: 'int2', is_nullable: 'YES', is_primary: false},
                {column_name: 'col2', udt_name: 'text', is_nullable: 'NO', is_primary: true}
            ]
            dbResponse.forEach(callback)
            assert.deepEqual(tableDefinition, {
                col1: { udtName: 'int2', nullable: true, primary: false },
                col2: { udtName: 'text', nullable: false, primary: true }
            })
        })
    })
